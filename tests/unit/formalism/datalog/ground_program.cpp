...
static_assert(std::same_as<View, fd::GroundProgramView>);
static_assert(std::constructible_from<Data,
                                      ...
                                      fd::GroundRuleViewList<f::PredicateTag>,
                                      fd::GroundRuleViewList<f::FunctionTag>>);
static_assert(requires(Data& data, const View& view) {
    ...
    data.goal;
    data.metric;
    data.predicate_ground_rules;
    data.function_ground_rules;
    data.clear();
...
TEST(TyrFormalismDatalogGroundProgram, RemovesStaticallyInapplicableRules)
{
    ...
    auto filtered = std::optional<fd::GroundProgramView> {};
    ...
    auto retained_function_source_index = ygg::uint_t(0);
    {
        ...
        const auto ground_atom = [&]<f::FactKind T>(fd::PredicateView<T> predicate_)
        {
            ...
            return intern(ygg::Data<fd::GroundAtom<T>> { binding_view.get_index() });
        };
        const auto ground_fterm = [&]<f::FactKind T>(fd::FunctionView<T> function_)
        {
            ...
            const auto binding_view = intern(std::move(binding));
            return intern(ygg::Data<fd::GroundFunctionTerm<T>> { binding_view.get_index() });
        };
        const auto static_literal = [&](fd::GroundAtomView<f::StaticTag> atom, bool polarity)
        { return intern(ygg::Data<fd::GroundLiteral<f::StaticTag>> { atom.get_index(), polarity }); };
        const auto fluent_literal = [&](fd::GroundAtomView<f::FluentTag> atom, bool polarity)
        { return intern(ygg::Data<fd::GroundLiteral<f::FluentTag>> { atom.get_index(), polarity }); };
        const auto condition = [&](std::initializer_list<fd::GroundLiteralView<f::StaticTag>> static_literals,
                                   std::initializer_list<fd::GroundLiteralView<f::FluentTag>> fluent_literals = {})
        {
            auto data = ygg::Data<fd::GroundConjunctiveCondition> {};
        ...
        const auto empty_lifted_body = intern(ygg::Data<fd::ConjunctiveCondition> {});
        const auto predicate_rule = [&](fd::GroundConjunctiveConditionView body, fd::GroundAtomView<f::FluentTag> head)
        {
            auto lifted_rule_data = ygg::Data<fd::Rule<f::PredicateTag>> {};
            ...
            auto binding_data = ygg::Data<f::RelationBinding<fd::Rule<f::PredicateTag>>> {};
            binding_data.relation = lifted_rule.get_index();
            const auto binding = intern(std::move(binding_data));

            auto rule_data = ygg::Data<fd::GroundRule<f::PredicateTag>> {};
        ...
        const auto function_rule = [&](fd::GroundConjunctiveConditionView body, fd::GroundFunctionTermView<f::FluentTag> head)
        {
            const auto source_fterm = lifted_fterm(head.get_function());
            const auto lifted_effect = intern(ygg::Data<fd::NumericEffect<f::FluentTag>> { f::NumericEffectOperatorKind::Assign,
                                                                                           source_fterm.get_index(),
                                                                                           ygg::Data<fd::FunctionExpression> { 1.0 } });
            auto lifted_rule_data = ygg::Data<fd::Rule<f::FunctionTag>> {};
            lifted_rule_data.body = empty_lifted_body.get_index();
            lifted_rule_data.head = ygg::Data<fd::NumericEffectOperator<f::FluentTag>> { f::NumericEffectOperatorKind::Assign, lifted_effect.get_index() };
            const auto lifted_rule = intern(std::move(lifted_rule_data));

            auto binding_data = ygg::Data<f::RelationBinding<fd::Rule<f::FunctionTag>>> {};
            binding_data.relation = lifted_rule.get_index();
            const auto binding = intern(std::move(binding_data));
            const auto effect = intern(ygg::Data<fd::GroundNumericEffect<f::FluentTag>> { f::NumericEffectOperatorKind::Assign,
                                                                                          head.get_index(),
                                                                                          ygg::Data<fd::GroundFunctionExpression> { 1.0 } });

            auto rule_data = ygg::Data<fd::GroundRule<f::FunctionTag>> {};
            rule_data.binding = binding.get_index();
            rule_data.body = body.get_index();
            rule_data.head = ygg::Data<fd::GroundNumericEffectOperator<f::FluentTag>> { f::NumericEffectOperatorKind::Assign, effect.get_index() };
            return intern(std::move(rule_data));
        };
        ...
        const auto static_value = intern(ygg::Data<fd::GroundFunctionTermValue<f::StaticTag>> { static_term.get_index(), 2.0 });
        const auto fluent_value = intern(ygg::Data<fd::GroundFunctionTermValue<f::FluentTag>> { retained_term.get_index(), 3.0 });
        const auto metric = intern(ygg::Data<fd::Metric> { ygg::Data<fd::GroundFunctionExpression> { retained_term.get_index() } });

        auto program_data = ygg::Data<fd::GroundProgram> {};
        ...
        program_data.predicate_ground_rules = { removed_predicate_rule.get_index(), retained_predicate_rule.get_index() };
        program_data.function_ground_rules = { removed_function_rule.get_index(), retained_function_rule.get_index() };
    ...
    ygg::visit(
        [&](auto value)
        {
            if constexpr (std::same_as<decltype(value), fd::GroundFunctionTermView<f::FluentTag>>)
    ...
    EXPECT_EQ(destination.size<fd::GroundRule<f::PredicateTag>>(), 1);
    EXPECT_EQ(destination.size<fd::GroundRule<f::FunctionTag>>(), 1);
}
