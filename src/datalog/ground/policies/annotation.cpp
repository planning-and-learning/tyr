...
{
    and_annot.insert_or_assign(head, BaseAnnotation<GroundTag>(Cost(0)));
}
...
{
    numeric_and_annot.insert(head, interval, BaseAnnotation<GroundTag>(Cost(0)));
}
...
template<typename AggregationFunction>
void AndAnnotationPolicy<GroundTag, AggregationFunction>::update_annotation(::tyr::formalism::datalog::GroundFunctionTermView<::tyr::formalism::FluentTag> head,
                                                                            ygg::ClosedInterval<ygg::float_t> interval,
    ...
    const AndAnnotationContext<GroundTag, ::tyr::formalism::PredicateTag>& context)
{
    m_achievers.update(head,
                       [&](auto& achievers, bool initialized)
                       {
                           if (!initialized)
                               achievers.clear();
                           achievers.emplace_back(context.rule, context.metric, context.current_cost, context.numeric_supports);
                       });
}
