...
StateView<GroundTag> StateRepository<GroundTag>::get_registered_state(ygg::Index<State<GroundTag>> state_index)
{
    ...
    m_fluent_backend.unpack(packed_state.template get_atoms<f::FluentTag>(), state_builder->template get_atoms<f::FluentTag>());
    m_derived_backend.unpack(packed_state.template get_atoms<f::DerivedTag>(), state_builder->template get_atoms<f::DerivedTag>());
    m_numeric_backend.unpack(packed_state.get_numeric_variables(), state_builder->get_numeric_variables());
...
{
    auto state_builder = m_state_builder_pool.get_or_allocate();
    state_builder->clear();

    state_builder->resize_fluent_facts(m_task->get_task().get_fluent_variables().size());
    state_builder->resize_derived_atoms(m_task->get_task().get_atoms<f::DerivedTag>().size());
