...
concept SearchPolicyConcept =
    ...
    } && (!T::supports_f_layer_synchronization || requires(const T& policy, const typename T::Options& options) {
        { T::synchronize_f_layers(options) } -> std::convertible_to<bool>;
        { policy.get_min_f_value() } -> std::same_as<ygg::float_t>;
