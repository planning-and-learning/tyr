...
parser = tfp.Parser("domain.pddl", parser_options)
# Instantiate a lifted task.
task = tpl.Task(parser.parse_task("problem.pddl", parser_options))
...
- `pyyggdrasil >= 0.2.2, < 0.3` for shared third-party native dependencies.
- `pypddl >= 1.2, < 1.3` for Loki's PDDL parser library, headers, and CMake package.
- `pypddl-datasets >= 0.0.12, < 0.1` for the PDDL benchmark data used by the C++ test and profiling fixtures (resolved from its cache at CMake configure time).
...
```console
python -m pip install 'pyyggdrasil>=0.2.2,<0.3' 'pypddl>=1.2.1,<1.3' 'pypddl-datasets>=0.0.12,<0.1'
