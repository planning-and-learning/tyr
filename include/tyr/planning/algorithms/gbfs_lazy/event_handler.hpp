...
/// Calls are serialized per logical worker but have no OS-thread affinity. State views retain the repository wrappers that produced them; these wrappers
/// may differ in hash-distributed mode and need not identify the logical owner in shared mode. A callback must not re-enter the search or wait for work from
