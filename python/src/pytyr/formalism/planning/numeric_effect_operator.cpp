...
void bind_numeric_effect_operator(nb::module_& m, RepositoryBinding& repository)
{
    ...
    {
        using V = ygg::Data<NumericEffectOperator<AuxiliaryTag>>;
        auto cls = nb::class_<V>(m, "AuxiliaryNumericEffectOperatorData").def(nb::init<V::ViewVariant<Repository>>(), "value"_a);
        ygg::add_print(cls);
        ygg::add_comparison(cls);
        ygg::add_hash(cls);
    }
    ...
    {
        using V = NumericEffectOperatorView<AuxiliaryTag>;
        auto cls = nb::class_<V>(m, "AuxiliaryNumericEffectOperator").def("get_variant", &V::get_variant);
        ygg::add_print(cls);
        ygg::add_comparison(cls);
        ygg::add_hash(cls);
    }
    ...
    repository.def("create", &create_data<NumericEffectOperator<AuxiliaryTag>>, "data"_a, nb::keep_alive<0, 1>(), nb::keep_alive<0, 2>());
}
