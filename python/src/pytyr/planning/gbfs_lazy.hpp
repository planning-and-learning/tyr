...
void bind_options(nb::module_& m, const std::string& name)
{
    ...
    nb::class_<T>(m, name.c_str())
        ...
        .def_rw("search_budget", &T::search_budget)
...
void bind_find_solution(nb::module_& m, const std::string& py_name)
{
    m.def(
        ...
        [](Task<Kind>& task,
           ...
           const Options<Kind>& options) { return find_solution(task, state_repository, axiom_evaluator, successor_generator, heuristic, options); },
